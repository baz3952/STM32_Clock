...
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode,u16 fontcolor, u16 backcolor)
{  							  
  u8 temp,t1,t;
	u16 y0=y;
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size,u16 fontcolor,u16 backcolor)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size,0,fontcolor,backcolor);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,0,fontcolor,backcolor); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode,u16 fontcolor,u16 backcolor)
{  
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)LCD_ShowChar(x+(size/2)*t,y,'0',size,mode&0X01,fontcolor,backcolor);  
				else LCD_ShowChar(x+(size/2)*t,y,' ',size,mode&0X01,fontcolor,backcolor);  
 				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,mode&0X01,fontcolor,backcolor); 
	}
} 
//显示字符串
//x,y:起点坐标
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p,u16 fontcolor, u16 backcolor)
{         
	u8 x0=x;
	width+=x;
	height+=y;
    ...
    {       
        ...
        LCD_ShowChar(x,y,*p,size,0,fontcolor, backcolor);
        x+=size/2;
        p++;
    }  
}
